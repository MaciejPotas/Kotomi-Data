"""Kivy presentation for the Adjectives quiz.
...
def mobile_sentence_hint_items(
    ...
    """Return compact vocabulary for the mobile Hint, never the answer."""
    ...
    for slot, identifier in bindings:
        ...
        item = None
        dictionary_id = preferred_dictionaries.get(slot, "")
        if getattr(slot_definition, "kind", "") == "entity" or slot == "subject":
            item = entities.get(identifier)
            dictionary_id = "entity"
        else:
            dictionary_id = dictionary_id or str(
    ...
    if not result:
        ...
        append_item(fallback, ("adjectives", question.adjective_ids))
    return result
...
def create_app_class():
    ...
    def settings_summary(settings: AdjectiveQuizSettings) -> str:
        types = []
        if settings.enable_i_adjectives:
            types.append(tr("specialized.i_adjectives"))
        if settings.enable_na_adjectives:
            types.append(tr("specialized.na_adjectives"))
        forms = ", ".join(
        ...
        return "\n".join(
            (
                ...
                f"{tr('specialized.adjectives')}: {', '.join(types)}; "
                f"{tr('specialized.in_sentence_count')}: {settings.adjective_count}",
    ...
    def mobile_height(base_height: float) -> float:
        return dp(scaled_button_height(base_height, current_button_scale()))

    class PolishLabel(Label):
        def __init__(self, **kwargs):
            kwargs.setdefault("font_name", MOBILE_FONT)
    ...
    class PolishButton(Button):
        def __init__(self, **kwargs):
            ...
            if base_height is not None:
                kwargs["height"] = mobile_height(float(base_height))
            if "font_size" in kwargs:
                kwargs["font_size"] = scaled_font_size(
                    kwargs["font_size"]
                )
    ...
    class MobileSpinnerOption(SpinnerOption):
        def __init__(self, **kwargs):
            kwargs.setdefault("font_name", MOBILE_FONT)
            kwargs.setdefault("height", mobile_height(48))
            super().__init__(**kwargs)
            self._mobile_base_height = 48

    def action_row(base_height: float, spacing: float = 8) -> BoxLayout:
        row = BoxLayout(
            size_hint_y=None,
            height=mobile_height(base_height),
            spacing=dp(spacing),
        )
        row._mobile_action_base_height = base_height
        return row
    ...
    def setting_row(
        ...
        row = BoxLayout(size_hint_y=None, height=mobile_height(64))
    ...
    class SettingsScreen(Screen):
        def __init__(self, **kwargs):
            ...
            self.content.add_widget(
                PolishLabel(
                    text=(
                        f"{tr('specialized.adjectives_title')} "
            ...
            self.mode_spinner = Spinner(
                text=mode_label(MODE_TRANSFORMATION),
                values=[mode_label(mode) for mode in MODE_LABELS],
                font_name=MOBILE_FONT,
                option_cls=MobileSpinnerOption,
                font_size=scaled_font_size(19),
                size_hint_y=None,
                height=64,
            )
            ...
            self._section(tr("specialized.adjectives"))
            self._boolean_row(
                "enable_i_adjectives",
                tr("specialized.i_adjectives"),
            )
            self._boolean_row(
                "enable_na_adjectives",
                tr("specialized.na_adjectives"),
            )

            self._section(tr("specialized.in_sentence_count"))
            self.adjective_count = self._number_input(
                tr("specialized.in_sentence_count"),
            )
            ...
            filter_help = PolishLabel(
                text=tr(
                    ...
                    example="type=i_adjective;id=oishii,hayai",
                ),
                font_size=15,
                halign="left",
                valign="middle",
            ...
            self.word_filter = TextInput(
                multiline=False,
                font_name=MOBILE_FONT,
                font_size=scaled_font_size(18),
                size_hint_y=None,
                height=64,
                hint_text="type=i_adjective | category=food | id=oishii",
            )
            ...
            button_scale_row = BoxLayout(
                orientation="vertical",
                size_hint_y=None,
                height=96,
                spacing=dp(4),
            )
            button_scale_title = BoxLayout(size_hint_y=None, height=36)
            button_scale_label = PolishLabel(
                text=tr("specialized.button_scale"),
                font_size=18,
                halign="left",
                valign="middle",
            )
            wrap_label(button_scale_label)
            self.button_scale_slider = Slider(
                min=0.8,
                max=1.6,
                step=0.05,
                value=DEFAULT_MOBILE_BUTTON_SCALE,
            )
            self.button_scale_value = PolishLabel(
                text="100%",
                font_size=18,
                size_hint_x=None,
                width=72,
            )
            self.button_scale_slider.bind(
                value=self._button_scale_changed
            )
            button_scale_title.add_widget(button_scale_label)
            button_scale_title.add_widget(self.button_scale_value)
            button_scale_row.add_widget(button_scale_title)
            button_scale_row.add_widget(self.button_scale_slider)
            self.content.add_widget(button_scale_row)
            ...
            buttons = BoxLayout(
                orientation="vertical",
                size_hint_y=None,
                height=mobile_height(136) + dp(8),
                spacing=dp(8),
            )
            buttons._mobile_action_base_height = 144
            save_row = action_row(68)
        ...
        def populate(self, settings: AdjectiveQuizSettings) -> None:
            ...
            self.adjective_count.text = str(settings.adjective_count)
            ...
            self.button_scale_slider.value = settings.mobile_button_scale
            self._button_scale_changed(
                self.button_scale_slider,
                settings.mobile_button_scale,
            )
            self.status.text = ""
            self.statistics.text = ""

        def read_settings(self) -> AdjectiveQuizSettings:
            settings = AdjectiveQuizSettings(
                ...
                enable_i_adjectives=self.value_checks[
                    "enable_i_adjectives"
                ].active,
                enable_na_adjectives=self.value_checks[
                    "enable_na_adjectives"
                ].active,
                adjective_count=_safe_int(self.adjective_count.text, 0),
                ...
                mobile_button_scale=float(
                    self.button_scale_slider.value
                ),
            )
            settings.validate()
            return settings
        ...
        def _button_scale_changed(
            self,
            _slider,
            value: float,
        ) -> None:
            self.button_scale_value.text = f"{round(value * 100)}%"
        ...
        def _statistics_ready(
            ...
            self.statistics.text = "\n".join(
                (
                    tr("specialized.available_adjectives", count=values["words"]),
                    tr("specialized.eligible_adjectives", count=values["eligible"]),
                    tr("specialized.active_patterns", count=values["active_patterns"]),
                    tr("specialized.available_entities", count=values["entities"]),
                    tr("specialized.noun_categories", count=values["categories"]),
                    tr("specialized.combination_spaces", count=values["spaces"]),
        ...
        def __init__(self, **kwargs):
            ...
            sections = (
                (
                    ...
                    tr("specialized.help.adjectives.how"),
                ),
                (
                    tr("specialized.help.keyboard"),
                    tr("specialized.help.adjectives.keyboard"),
                ),
                (
                    tr("specialized.help.hint"),
                    tr("specialized.help.adjectives.hint"),
                ),
                (
                ...
                (
                    ...
                    tr("specialized.help.adjectives.filters"),
                ),
                (
    ...
    class QuizScreen(Screen):
        def __init__(self, **kwargs):
            super().__init__(**kwargs)
            self.session: Optional[AdjectiveQuizSession] = None
            ...
            header = action_row(MOBILE_QUIZ_LAYOUT.header_height, spacing=6)
            ...
            self.primary_button.bind(
                on_release=lambda *_: self.primary_action()
            )
            ...
            self.secondary_button.bind(
                on_release=lambda *_: self.secondary_action()
            )
            ...
            # No ScrollView here: active quiz controls stay pinned from the
            # top and reserved feedback slots keep every state the same size.
            # Overflow on an unusually short keyboard viewport is clipped at
            # the bottom, never by pushing the question and input upward.
            content_anchor = AnchorLayout(anchor_x="left", anchor_y="top")
            content_anchor.add_widget(content)
            root.add_widget(content_anchor)
            self.add_widget(root)

        def set_session(self, session: AdjectiveQuizSession) -> None:
        ...
        def check_answer(self) -> None:
            ...
            if result.state == "correct":
                ...
                self.status.set_fitted_text(escape_markup(message), message)
                self.status.color = accent_bad
            if result.state in {"correct", "wrong", "retry"}:
                self.keep_keyboard_visible()
        ...
        def _auto_advance(self, _dt: float) -> None:
            self.auto_event = None
            if self._focus_is_active():
                self.next_question()
        ...
        def toggle_hint(self) -> None:
            ...
            self.hint.set_fitted_text(escape_markup(hint_text), hint_text)
            self.hint.opacity = 1
            self.configure_actions()
            self.keep_keyboard_visible()

        def show_hint(self) -> None:
            """Compatibility alias for launcher integrations."""
            self.toggle_hint()

        def _fit_hint(self, *_args) -> None:
            if not self.hint_visible or not self.hint.text:
                self.hint.opacity = 0
                return
            self.hint._fit_font_to_width()

        def _reveal_hint(self, _dt: float) -> None:
            self._fit_hint()
    ...
    class KotomiAdjectivesApp(App):
        ...
        def build(self):
            ...
            self.engine: Optional[AdjectiveQuizEngine] = None
            self.engine_error = ""
            self.update_message = ""
        ...
        def _resize_mobile_controls(
            ...
            if button_height is not None:
                widget.height = dp(
                    scaled_button_height(button_height, scale)
                )
            row_height = getattr(
                widget,
            ...
            if row_height is not None:
                widget.height = dp(
                    scaled_button_height(row_height, scale)
                )
            for child in getattr(widget, "children", []):
    ...
    return KotomiAdjectivesApp


def create_embedded_controller():
    """Create the adjective UI controller for the Kotomi master app."""
